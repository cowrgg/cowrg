        ...
        {CATEGORIES.map((cat) => (
          <div key={cat} className="rounded-2xl border border-border bg-card p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
              <Input
                value={descs[cat]?.image_url || ""}
                onChange={(e) => update(cat, "image_url", e.target.value)}
                placeholder="آدرس عکس دسته (https://...)"
              />
              {descs[cat]?.image_url && (
                <div className="h-16 w-16 overflow-hidden rounded-lg border border-border bg-secondary">
                  <Image src={descs[cat].image_url} alt={cat} className="h-full w-full object-cover" fittingType="fill" />
                </div>
              )}
            </div>
